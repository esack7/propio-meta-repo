...
- [x] G3: Record policy/approval and cancellation decisions, reviewed-argument fingerprints, mode and per-invocation skill scope, and provider request mutations.
...
## Current checkpoint (2026-09-22, implementation merged)
...
- [Providers PR #16](https://github.com/esack7/propio-providers/pull/16) merged the explicit nine-adapter scenario matrix: local build, 411 tests, formatting, Fallow, and all PR checks passed. [Agent PR #100](https://github.com/esack7/propio-agent/pull/100) merged stable skill-invocation lineage, ephemeral keyed reviewed-argument fingerprints, and distinct cancellation decisions without duplicate deny/cancel outcomes: local build, 1,634 tests, formatting, Fallow, and all PR checks passed.
...
- Remaining gates: review and merge the evidence PR; then run `close-spec` when the worktrees are no longer needed.
